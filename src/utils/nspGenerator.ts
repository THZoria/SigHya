...
import { processAnimationForNSP, processImageForNSP, processLogoForNSP } from './imageProcessor'
...
export async function generateNSP(options: NSPGeneratorOptions): Promise<Blob> {
  try {
    ...
    const imageBlob = options.imageFile ? await processImageForNSP(options.imageFile) : null

    if (!imageBlob) {
      throw new Error('Image file is required')
    }

    const logoBlob = options.logoFile ? await processLogoForNSP(options.logoFile) : null
    ...
    throw new Error(
      `NSP generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`,
    )
  }
}
